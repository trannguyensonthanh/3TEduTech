...
from src.core.gemini import generate_chat_response, generate_suggested_questions, generate_chat_response_stream
...
async def _handle_buy_course(query: str, keywords: list[str], chat_history: list[dict] | None = None) -> dict:
    ...
    # 1. Kiểm tra xem người dùng có chọn đích danh một khóa học từ nút UI (có dấu ':')
    ...
    # 3. Nếu là click nút thanh toán tường minh hoặc tìm thấy tên khóa học thật chính xác trong hệ thống -> Mở thanh toán!
    ...
    async def event_generator():
        try:
            if intent == UserIntent.BUY_COURSE:
                ...
                if not search_results:
                    yield f"event: token\ndata: {json.dumps({'text': 'Xin lỗi, tôi không tìm thấy khóa học nào phù hợp với yêu cầu của bạn. Hãy thử mô tả mục tiêu học tập của bạn theo cách khác nhé!'}, ensure_ascii=False)}\n\n"
                else:
            ...
            yield f"event: token\ndata: {json.dumps({'text': '\n\n[Xin lỗi, đã xảy ra gián đoạn trong kết nối AI Streaming]'}, ensure_ascii=False)}\n\n"
