...
const AiMasterChat: React.FC = () => {
  ...
  return (
    <Layout>
      <div className={`flex flex-col md:flex-row bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 min-h-[calc(100vh-64px)] ${isFullScreen ? 'fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 h-screen w-screen' : ''}`}>
        
        {/* --- LEFT SIDEBAR (SESSIONS) --- */}
        <div
          className={`${
            sidebarOpen ? 'w-full md:w-80 opacity-100' : 'w-0 opacity-0 pointer-events-none md:pointer-events-auto'
          } transition-all duration-300 ease-in-out bg-white dark:bg-slate-900/90 border-r border-slate-200 dark:border-slate-800/80 backdrop-blur-xl flex flex-col shrink-0 relative overflow-hidden`}
        >
          {/* Header & New Chat Button */}
          <div className="p-4 border-b border-slate-200 dark:border-slate-800/80 flex flex-col gap-3">
            <Button
              onClick={handleNewSession}
              className="w-full h-11 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm shadow-md rounded-xl transition-all active:scale-95 flex items-center justify-center gap-2"
            >
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9 h-9 bg-slate-100 dark:bg-slate-950/60 border-slate-200 dark:border-slate-800 text-xs rounded-lg text-slate-700 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-indigo-500/50"
              />
            </div>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`group p-3 rounded-xl cursor-pointer transition-all duration-200 flex items-center justify-between gap-2 border ${
                      isActive
                        ? 'bg-indigo-50/80 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 shadow-sm'
          ...
          {/* Footer Info */}
          <div className="p-3 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/40 text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
            <div className="flex items-center gap-1.5 font-semibold text-slate-600 dark:text-slate-400">
              <ShieldCheck className="w-4 h-4" />
              <span>AI Learning Master</span>
            </div>
            <span className="bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded text-[10px] font-mono">v3.2</span>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-950 relative overflow-hidden">
          ...
          {/* Top Navbar */}
          <div className="h-14 border-b border-slate-200 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md px-4 flex items-center justify-between gap-3 shrink-0 z-10 shadow-sm">
            <div className="flex items-center gap-2">
              <button
                onClick={() => setSidebarOpen(!sidebarOpen)}
                className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white transition-colors"
              ...
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-800 dark:text-white truncate max-w-[200px] sm:max-w-md">
                  {activeSession.title}
                </h3>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <div className="hidden sm:flex items-center gap-1 bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 p-1 rounded-xl">
                <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 px-2">Kích cỡ:</span>
              ...
              {messages.map((message, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Avatar */}
                    {!isUser && (
                      <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-slate-700 dark:text-slate-300 shadow-sm shrink-0 border border-slate-300 dark:border-slate-700">
                    ...
                    {/* Bubble */}
                    <div
                      className={`relative rounded-2xl p-4 sm:p-5 max-w-[90%] sm:max-w-[85%] leading-relaxed shadow-sm border ${
                        isUser
                          ? 'bg-slate-800 text-slate-50 rounded-br-none border-slate-700'
                          : 'bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-bl-none border-slate-200 dark:border-slate-800'
                      }`}
                    >
                      {/* Bot Header Action Bar */}
                      {!isUser && (
                        <div className="flex items-center justify-between gap-4 pb-2.5 mb-3 border-b border-slate-200 dark:border-slate-800/80 text-xs text-slate-500 font-semibold">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 transition-colors flex items-center gap-1 text-[11px]"
                      ...
                      {message.sources && message.sources.length > 0 && (
                        <div className="mt-4 bg-slate-100 dark:bg-slate-950/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800 text-xs">
                          <span className="text-slate-700 dark:text-slate-400 font-bold flex items-center gap-1.5 mb-1.5">
                            <BookOpen className="w-3.5 h-3.5" />
                            Nguồn Trích Xuất & Tri Thức Khóa Học:
                          </span>
                          <div className="space-y-1">
                            {message.sources.map((src, i) => (
                              <div key={i} className="text-slate-600 dark:text-slate-300 truncate font-mono text-[11px] bg-slate-200/50 dark:bg-slate-900 px-2 py-1 rounded">
                                • {src.file_name}
                              </div>
                            ))}
                          </div>
