      ...
      <div className='min-h-screen -m-6 p-6'>
        ...
        <div className='relative z-10 space-y-8'>
          ...
          <div>
            ...
            <p className='text-muted-foreground mt-1'>Track your teaching performance and revenue</p>
          </div>
          ...
          <Tabs value={period} onValueChange={(v) => setPeriod(v as 'weekly' | 'monthly')} className='space-y-6'>
            ...
            <TabsContent value={period} className='space-y-6'>
                ...
                <div className='space-y-3'>
                  {isLoading ? (
                    ...
                    coursePerformance.map((course, i) => (
                      <div key={course.courseId} className='flex items-center gap-4 p-4 rounded-xl bg-muted/50 hover:bg-muted transition-all group border border-transparent hover:border-border'>
                        <div className='flex-shrink-0 w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center  font-bold text-sm shadow-lg'>
                          {i + 1}
                        </div>
                        <div className='flex-1 min-w-0'>
                          <Link to={`/courses/${course.slug}`} className=' font-medium hover:text-indigo-400 transition-colors truncate block'>
                            {course.courseName}
                          </Link>
                        ...
                        <div className='text-right hidden sm:block'>
                          ...
                          <div className='flex items-center gap-2 mt-1'>
                            ...
                            <span className='text-xs text-muted-foreground'>{course.avgCompletion.toFixed(0)}%</span>
                          </div>
                        </div>
                      </div>
                    ))
                  ) : (
                    <p className='text-center text-muted-foreground py-12'>No course performance data yet. Publish courses to see analytics!</p>
                  )}
                </div>
              </div>
              ...
              <div className='grid gap-6 lg:grid-cols-3'>
                <div className='lg:col-span-1 rounded-xl border bg-card text-card-foreground shadow-sm p-6 flex flex-col justify-between relative overflow-hidden'>
                  ...
                  <div>
                    ...
                    <h3 className='text-2xl font-bold'>Sức Khỏe Bài Kiểm Tra</h3>
                  ...
                  <div className='my-6 grid grid-cols-2 gap-3'>
                    <div className='p-4 rounded-xl bg-muted/50 border text-center'>
                      <span className='text-xs text-muted-foreground block mb-1'>Tỷ Lệ Thi Đỗ</span>
                      <span className='text-2xl font-boldtext-emerald-400'>{quizStats.avgPassRate}%</span>
                    </div>
                    <div className='p-4 rounded-xl bg-muted/50 border text-center'>
                      <span className='text-xs text-muted-foreground block mb-1'>Điểm TB (10đ)</span>
                      <span className='text-2xl font-boldtext-amber-400'>{quizStats.avgScore}</span>
                    </div>
                  </div>
                ...
                <div className='lg:col-span-2 rounded-xl border bg-card text-card-foreground shadow-sm p-6 flex flex-col justify-between'>
                  <div>
                    <div className='flex items-center justify-between mb-4'>
                      <div>
                        ...
                        <p className='text-xs text-muted-foreground'>Các câu hỏi trắc nghiệm khiến học viên trả lời sai nhiều nhất</p>
                      </div>
                    ...
                    <div className='space-y-3.5 mt-4'>
                      {quizStats.hardestQuestions.map((q, idx) => (
                        <div key={q.id} className='p-4 rounded-xl bg-muted/50 hover:bg-muted transition-all border border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
                          <div className='flex items-start gap-3 min-w-0'>
                            ...
                            <div className='min-w-0'>
                              <p className='text-sm font-semibold text-foreground line-clamp-1' title={q.question}>{q.question}</p>
              ...
              <div className='rounded-xl border bg-card text-card-foreground shadow-sm p-6 relative overflow-hidden'>
                ...
                <div className='flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 relative z-10'>
                  <div>
                    ...
                    <h3 className='text-xl font-bold '>Các Điểm Nghẽn Cổ Chai Học Tập</h3>
                    <p className='text-sm text-muted-foreground'>Nơi học viên thường xuyên bị đứt quãng hoặc bỏ lỡ bài học nhất trong chương trình</p>
                  </div>
                  <div className='flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 p-3 rounded-xl border border-border max-w-sm'>
                ...
                <div className='grid gap-4 md:grid-cols-3 relative z-10'>
                  {dropoutBottlenecks.map((item) => (
                    <div key={item.id} className='p-5 rounded-xl bg-muted/50 border hover:border-red-500/30 transition-all flex flex-col justify-between'>
                      <div>
                        <div className='flex items-center justify-between mb-2'>
                          <span className='text-xs font-semibold px-2 py-0.5 rounded bg-muted text-indigo-300 truncate max-w-[150px]'>
                            {item.courseName}
                          </span>
                          <span className='text-xs font-boldtext-red-400 bg-red-500/10 px-2 py-0.5 rounded-full border border-red-500/20'>
                            {item.dropRate}% gián đoạn
                          </span>
                        </div>
                        <p className='font-semibold text-foreground text-sm mt-2 line-clamp-2'>{item.lessonTitle}</p>
                      </div>
                ...
                <div className='rounded-xl border bg-card text-card-foreground shadow-sm p-6 flex flex-col justify-between'>
                  <div>
                    <div className='flex items-center justify-between mb-4'>
                      ...
                      <span className='text-xs text-muted-foreground font-medium'>Tổng {sentiment.stars.reduce((acc, curr) => acc + curr.count, 0)} lượt</span>
                    </div>

                    <div className='space-y-2.5 mt-4'>
                      {sentiment.stars.map((s) => (
                        <div key={s.stars} className='flex items-center gap-3 text-sm'>
                          <div className='flex items-center gap-1 w-12 font-medium text-muted-foreground'>
                            <span>{s.stars}</span>
                          ...
                          <span className='w-12 text-right text-xs font-semibold text-muted-foreground'>{s.percentage}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    <div className='flex items-center justify-between mb-4'>
                      <div>
                        ...
                        <p className='text-xs text-muted-foreground'>Các câu hỏi mới nhất từ học viên cần sự trợ giúp từ bạn</p>
                      </div>
                    ...
                    <div className='space-y-3 mt-4'>
                      {sentiment.recentTopics.map((t) => (
                        <div key={t.id} className='p-3.5 rounded-xl bg-muted/50 hover:bg-muted transition-all border border-border'>
                          <div className='flex items-center justify-between gap-2 mb-1.5'>
                            <div className='flex items-center gap-2'>
                              <span className='font-bold text-foreground text-xs'>{t.student}</span>
                          ...
                          <p className='text-sm text-muted-foreground font-medium line-clamp-1'>{t.title}</p>
                        </div>
                      ))}
                    </div>
                  </div>
