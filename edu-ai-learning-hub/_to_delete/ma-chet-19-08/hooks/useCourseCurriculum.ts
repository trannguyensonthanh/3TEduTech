...
type CurriculumAction =
  ...
  | {
      type: 'ADD_LESSON';
      /* [SỬA 19/08/2026] `lessonOrder` KHÔNG bắt buộc ở đây: chính reducer
         tính nó từ số bài đang có trong chương (xem case 'ADD_LESSON'). Bắt
         buộc nó ở payload khiến addLesson() — vốn nhận Omit<..., 'lessonOrder'>
         — không truyền vào được (TS2741). */
      payload: {
        sectionId: number | string;
        lesson: Omit<Lesson, 'lessonOrder'> & { lessonOrder?: number };
      };
    } // Lesson đã có tempId
  | {
      type: 'UPDATE_LESSON';
