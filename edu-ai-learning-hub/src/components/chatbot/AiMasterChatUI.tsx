...
export interface AiMasterChatUIProps {
...
const AiMasterChatUI: React.FC<AiMasterChatUIProps> = ({ layoutMode, onClose, isMaximized: externalIsMaximized, onToggleMaximize }) => {
    ...
    <>
      <div
        className={`flex flex-1 min-h-0 flex-col bg-background text-foreground md:flex-row ${
          layoutMode === 'full' && isFullScreen ? 'fixed inset-0 z-50 h-screen w-screen' : 'h-full w-full'
        }`}
      >
              ...
              {layoutMode === 'full' ? (
                ...
                onToggleMaximize && (
                  <button
                    ...
                    className="rounded-md p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                    title={externalIsMaximized ? 'Thu nhỏ' : 'Phóng to'}
                  >
                    {externalIsMaximized ? (
                      <Minimize2 className="h-4 w-4" aria-hidden="true" />
                    ) : (
